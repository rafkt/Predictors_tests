...
//		float curValue = (weightLevel * 1f) + (weightLength * 1f) + (weightDistance * 0.0001f);
	...
	// 				for (int sub_i = 0; sub_i < current_total_subs + 1; sub_i++){ // I have to sequentially forgive substitutions - start with 0 then with 1, 2...
	...
	// 						if (subs < 0 || current_total_subs - subs > sub_i || seq_i >= 0) continue; //update smith-water jar.. after this line you can add the consequent of ret_sequence to the cout table.
		...
		for (int i = 0; i < sequence.length - 1; i ++){
			...
			for( List<Item> subseq_variation_del : SequenceVariations.getOneDeletionAnywhere(curr_subseq_variation)){
				...
				for(List<Item> subseq_variation_del_perm : SequenceVariations.getPairPermutations(subseq_variation_del)){
					subseq = subseq_variation_del_perm.toArray(new Item[subseq_variation_del_perm.size()]);
